          ...
          {user ? (
            <div className="relative" ref={dropdownRef}>
              <button
                ...
                className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center text-[#1D2D50] font-bold hover:ring-2 hover:ring-[#1D2D50]/30 transition-all"
              >
                {user.name ? user.name.charAt(0).toUpperCase() : "U"}
              </button>

              {dropdownOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-white border rounded-lg shadow-lg py-1 transition-all duration-200 ease-in-out">
                  <Link
                    to="/profile"
